...
//-------------------------------------------------------------
//function declaration for condition_check
long condition_check(int cmd, int syscall, int pid);
...
void my_exit_group(int status)
{
    ...
    int exit_pid = current->pid;
	//checking over all the system call's monitored list, and removing exit_pid
 ...
 *      - REQUEST_INTERCEPT to intercept the 'syscall' argument
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	long errcheck = 0;
	errcheck = condition_check(cmd,syscall,pid);
 	...
 	if(errcheck == 0){
  		...
  		if (cmd == REQUEST_START_MONITORING){
  			//monitor all 
			if(pid == 0){
				//since we are modifiying table
  				spin_lock(&pidlist_lock);
				if(table[syscall].monitored != 2){
					table[syscall].monitored = 2;
					//clear monitored list, it becomes a blacklist (list of pids to not monitor)
			...
			//not monitor all	
				...
				//check to see if we are adding a PID for a system call monitoring all PIDs, return EBUSY if it is not in blacklist
				if(table[syscall].monitored == 2){
					if(check_pid_monitored(syscall,pid) == 0){ //not in blacklist
						return -EBUSY;
					}
					else{//pid was in blacklist, so we remove it and it becomes monitored
						del_pid_sysc(syscall,pid);
					}
				}
				else{
					// check if the pid existed
					if(add_pid_sysc(pid, syscall) != 0  ){
						//must be before the return, but looks ugly like this. 
						spin_unlock(&pidlist_lock);
						return -ENOMEM;
					}
 		...
 		if (cmd == REQUEST_STOP_MONITORING){
			...
			//stop monitoring one PID
				...
				if(table[syscall].monitored == 2){ //has a blacklist, add PID to blacklist
  		...
  		if (cmd == REQUEST_SYSCALL_INTERCEPT){
			...
			//since we are modifiying table
  			spin_lock(&pidlist_lock);
			//since we are modifiying sys_call_table
  			spin_lock(&calltable_lock);
			
			//changing it to be able to write
			set_addr_rw((unsigned long)(sys_call_table + syscall));			
			
			//change it so that the table has it as intercepted
      			table[syscall].intercepted = 1;
			//Put the orignal syscall in to table so we can use it later
    			table[syscall].f = sys_call_table[syscall];
			
			//intercept and the original syscall
			sys_call_table[syscall] = &interceptor;	
	
			//changing it to read only again
   		...
   		if (cmd == REQUEST_SYSCALL_RELEASE){
			...
			set_addr_rw((unsigned long)(sys_call_table + syscall));
   			...
   			//change it so that the table is not intercepted
      		table[syscall].intercepted = 0;
			table[syscall].monitored =0;

			//Put the orignal syscall back
...
long condition_check(int cmd, int syscall, int pid){
	...
	//criteria a)
	//syscall cant be below zero, cant be greater than the last syscall and cant be MY_CUSTOM_SYSCALL
	if(syscall < 0 || syscall > NR_syscalls || syscall == MY_CUSTOM_SYSCALL){
		return -EINVAL;
	}
	
	//criteria b)
	//is_valid_check is NULL if pid is not valid
	
    if(cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING){
	//pid is smaller than zero or not a valid process
	...
	if(cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE){
		//printk(KERN_ALERT"syscall intercept or syscall release");
		if(current_uid() != 0){ // if we are not root
			return -EPERM;
		}
	}
	...
	if(cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING){
		if(current_uid() != 0){ // if we are not root
			//so can not monitor all
			if(pid == 0){
				return -EPERM;
			}			
			//checking if calling process and current process has the same owner
	...
	//checking correct context of commands
	//condition a)Cannot de-intercept a system call that has not been intercepted yet.
	if(cmd == REQUEST_SYSCALL_RELEASE){
	...
	//condition b) Cannot stop monitoring for a pid that is not being monitored, or if the system call has not been intercepted yet.
	
	if ((pid_task(find_vpid(pid), PIDTYPE_PID) != NULL ) && (&syscall != NULL)){
		if(cmd == REQUEST_STOP_MONITORING){
			if(pid == 0){
				//already not monitored all
				if(table[syscall].monitored == 0){
					return -EINVAL;
				}
			}		
			if((check_pid_monitored(syscall, pid) == 0 )|| table[syscall].intercepted != 1) {
				return -EINVAL;
			}
		}
	}
	// condition c) Cant start monitoring a pid if its not been intercepted yet
	if(cmd == REQUEST_START_MONITORING){
	...
	//Checking for -EBUSY conditions:
	if (cmd == REQUEST_SYSCALL_INTERCEPT){
		//condition a) If intercepting a system call that is already intercepted.
		if(table[syscall].intercepted == 1){
	...
	if((pid_task(find_vpid(pid), PIDTYPE_PID) != NULL ) && (&syscall != NULL)){
		if(cmd == REQUEST_START_MONITORING){
			...
			//condition b) If monitoring a pid that is already being monitored.
			if(pid == 0){
...
static int init_function(void) {
	...
	//initialization of mytable structs
